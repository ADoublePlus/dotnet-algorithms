...
namespace Algorithms.Graphs
{
    public static class GraphTraversal
    {
        ...
        public static void DFS(int[][] adjacencyList, int start, Action<int> visitor)
        {
            ...
            HashSet<int> discoveredVertexIndices = new HashSet<int>();
            ...
            while (s.Count > 0)
            {
                ...
                if (!discoveredVertexIndices.Contains(current))
                {
                    discoveredVertexIndices.Add(current);
                    visitor(current);
        ...
        public static void DFS(Vertex[] vertices, Vertex start, Action<Vertex> visitor)
        {
            ...
            while (s.Count > 0)
            {
                ...
                if (!discoveredVertexIndices.Contains(index))
                {
                    discoveredVertexIndices.Add(index);
                    visitor(current);
        ...
        private static void BFS(
            Vertex[] vertices,
            Queue<int> level,
            HashSet<int> discoveredVertexIndices,
            ...
            while (level.Count > 0)
            {
                ...
                if (!discoveredVertexIndices.Contains(current))
                {
                    ...
                    for (int i = 0; i < totalEdges; i++)
                    {
                        ...
                        if (!discoveredVertexIndices.Contains(v.Index))
                        {
            ...
            while (level.Count > 0)
            {
                ...
                if (!discoveredVertexIndices.Contains(current))
                {
                    ...
                    for (int i = 0; i < totalEdges; i++)
                    {
                        ...
                        if (!discoveredVertexIndices.Contains(v))
                        {
                            nextLevel.Enqueue(v);
                        }
                    }
            ...
            Vertex start)
        {
            ...
            Action<Vertex> addNeighbours = (node) =>
            {
                int u = node.Index;
